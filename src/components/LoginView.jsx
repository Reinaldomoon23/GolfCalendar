...
export default function LoginView({ onLogin }) {
    ...
    const handleSubmit = async (e) => {
        ...
        const endpoint = isRegister ? 'create_user.php' : 'login.php';
        const body = isRegister
            ? { username, password, full_name: fullName, federation_id: federationId }
            : { username, password };

        try {
            const res = await fetch(`${baselink}/api/${endpoint}`, {
                method: 'POST',
                headers: {

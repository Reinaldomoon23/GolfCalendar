  ...
  return (
    <header className="app-header" style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '5px', marginBottom: '10px' }}>
        ...
        <span style={{ fontSize: '0.8em', opacity: 0.7, color: 'var(--color-text-muted)' }}>(v2.5.1)</span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', marginTop: '12px' }}>
        ...
        >
          ...
          <span>{isUpdatingHandicap ? 'Actualizando...' : (handicap && String(handicap).trim() !== '' ? String(handicap).substring(0, 15) : 'Actualizar hándicap')}</span>
        </button>

        {(pdfUrl || user?.federation_id) && (
          <button
            className="handicap-btn fade-in"
            onClick={(e) => {
              e.stopPropagation();
